...
class BillboardGeometry extends Geometry {
  ...
  // PATCH (local fork): persistent instance buffer.
  //
  // Upstream re-uploads the whole instance array through a transient buffer on
  // EVERY bind, whether or not anything changed — 1.12 MB per frame at 20k
  // instances, ~67 MB/s at 60fps, for a completely static batch.
  //
  // Keep the data GPU-resident instead and re-upload only when setInstance or
  // commit actually touched it.
  gpu.DeviceBuffer? _instanceBuffer;
  int _instanceBufferBytes = 0;
  bool _instanceDirty = true;
  ...
  void setInstance(
    ...
    _instanceDirty = true; // PATCH: mark for re-upload
  }
  ...
  void bind(
    ...
    //
    // PATCH (local fork): keep these GPU-resident across frames. Upstream
    // called `instanceTransients.emplace(liveBytes)` here unconditionally,
    // re-uploading the whole array every frame even when nothing changed.
    if (_instanceCount > 0) {
      ...
      final needed = liveBytes.lengthInBytes;

      // (Re)allocate only when the live range outgrows the current buffer.
      if (_instanceBuffer == null || _instanceBufferBytes < needed) {
        _instanceBuffer = gpu.gpuContext.createDeviceBuffer(
          gpu.StorageMode.hostVisible,
          needed,
        );
        _instanceBufferBytes = needed;
        _instanceDirty = true;
      }
      if (_instanceDirty) {
        _instanceBuffer!.overwrite(liveBytes);
