...
base class Scene implements SceneGraph {
  ...
  // PATCH (local fork): the ui.Image produced each frame by
  // `swapchainColor.asImage()` was never disposed.
  //
  // On Impeller that is harmless — asImage() wraps the existing texture. On the
  // WebGL2 shim every call allocates a fresh ImageBitmap-backed GPU image
  // (OffscreenCanvas.transferToImageBitmap), so at 60fps this leaks ~75 MB/s at
  // dpr 1 and ~680 MB/s at dpr 3. Desktop GC absorbs it; iOS Safari has a
  // ~300-500 MB WebGL ceiling and kills the tab in about five seconds.
  //
  // Disposing the image immediately after drawImageRect is unsafe: the recorded
  // picture is rasterized later. So hold it one frame and release it at the
  // start of the next present, by which point the previous frame is on screen.
  ...
  void _renderViewToCanvas({
    ...
    // PATCH (local fork): release last frame's presented image before minting
    // this frame's. See _previousPresentedImage for why this is deferred by one
    // frame rather than disposed immediately after drawImageRect.
