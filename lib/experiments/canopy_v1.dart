// PRESERVED FOR REFERENCE — the leaf-canopy version of the hero, as it stood
// at commit 5c44587 before the skeleton rewrite.
//
// Kept because the work in here is not obsolete, only paused: 20,000 instanced
// billboard leaves with an alpha-cut procedural leaf texture, per-leaf depth
// shading, wind animation, and a translucency pass following Filament's
// subsurfaceColor idea (a leaf between viewer and light glows warm rather than
// falling into shadow). All of it measured: bloom, wind and the leaf texture
// each cost nothing; fill rate is what costs frames.
//
// It is NOT wired into the app. `TreeHero` here is superseded by the skeleton
// stage in lib/main.dart. When the trunk and branches are right, the canopy
// comes back — hung on the branches this time rather than filling a sphere.
//
// Renamed to avoid clashing with the live widget.
import 'dart:async';
...
class CanopyV1 extends StatefulWidget {
  const CanopyV1({super.key});

  @override
  State<CanopyV1> createState() => _CanopyV1State();
}

class _CanopyV1State extends State<CanopyV1>
  ...
  void _onTick(Duration elapsed) {
    ...
    for (var i = 0; i < _leafCount; i++) {
      ...
      final transmit = backlight * math.pow(1 - _depth[i], 3).toDouble() * 0.40;
